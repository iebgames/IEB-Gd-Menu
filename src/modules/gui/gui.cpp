...
namespace eclipse::gui {
    ...
    CustomComponent* MenuTab::addCustom(std::string id, std::function<void()> drawCallback) {
        return static_cast<CustomComponent*>(this->addComponent(
            std::make_unique<CustomComponent>(std::move(id), std::move(drawCallback))
        ));
    }
    ...
    MenuTab& Engine::findTab(std::string_view name) {
        ...
        static constexpr std::array<std::string_view, 12> builtInTabs = {
            "tab.global", "tab.level", "tab.bypass", "tab.player", "tab.bot", "tab.creator",
            "tab.labels", "tab.shortcuts", "tab.keybinds", "tab.interface", "tab.recorder", "tab.ai-scripts"
        };
