...
#include <modules/gui/scripting/script_ui.hpp>
...
namespace eclipse::gui::imgui {
    ...
    void ImGuiRenderer::draw() {
        auto& io = ImGui::GetIO();
        auto scale = io.DisplaySize.x / 1920.f;
        config::setTemp("ui.scale", scale);

        ThemeManager::get()->updateChroma(io.DeltaTime);
        ...
        scripting::drawAISidebar();
        scripting::drawScriptStudio();
        renderPopups();
        #ifdef ECLIPSE_DEBUG_BUILD
        showTracingPopup();
        #endif
        ...
        // Watermark
        ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x - 10 * scale, io.DisplaySize.y - 10 * scale), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
        if (ImGui::Begin("Watermark", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoMove)) {
            ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 0.4f), "IEB Menu \xc2\xa9 IEB Games\n(Thanks to Eclipse Codebase)");
        }
        ImGui::End();
        m_insideDraw = false;
