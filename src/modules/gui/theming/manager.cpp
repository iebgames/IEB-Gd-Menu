...
namespace eclipse::gui {
    ...
    bool ThemeManager::loadTheme(std::filesystem::path const& path) {
        ...
        // chroma
        try_assign(m_chromaEnabled, other, "chromaEnabled");
        try_assign(m_chromaSpeed, other, "chromaSpeed");
    ...
    void ThemeManager::applyValues(matjson::Value& json, bool flatten) const {
        ...
        other["chromaEnabled"] = m_chromaEnabled;
        other["chromaSpeed"] = m_chromaSpeed;
    ...
    void ThemeManager::updateChroma(float dt) {
        if (!m_chromaEnabled) return;

        static float hue = 0.f;
        hue += dt * m_chromaSpeed * 0.1f;
        if (hue > 1.f) hue -= 1.f;

        auto color = Color::fromHSV(hue, 1.f, 1.f, 1.f);
