...
$on_mod(Loaded) {
    ...
    // Add "Interface" tab to edit theme settings
    {
        ...
        // chroma settings
        auto chromaToggle = tab->addToggle("interface.chroma", "chromaEnabled");
        chromaToggle->addOptions([](auto opt) {
            opt->addInputFloat("interface.chroma-speed", "chromaSpeed", 0.0f, 20.f, "%.1f")
               ->setDescription("interface.chroma-speed.desc");
        });
        chromaToggle->disableSaving();
        chromaToggle->setFlags(ComponentFlags::DisableCocos);
    ...
    // Add "AI & Custom Scripts" tab
    {
        using namespace gui;
        auto tab = MenuTab::find("tab.ai-scripts");
        
        tab->addButton("ai.script-maker")->callback([] {
            scripting::toggleAISidebar();
        })->setDescription("ai.script-maker.desc");

        tab->addButton("custom.script-studio")->callback([] {
            scripting::toggleScriptStudio();
        })->setDescription("custom.script-studio.desc");
    }
